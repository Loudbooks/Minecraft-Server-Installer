...
async fn main() {
    ...
    if downloaders::java::download(&client, is_arm, java_path).await.is_err() {
    ...
    if server_type.parse::<i32>().unwrap() == 1 {
        downloaders::vanilla::Vanilla::download(client.clone()).await.expect("Failed to download Vanilla.");
    } else if server_type.parse::<i32>().unwrap() == 2 {
        downloaders::paper::Paper::download(client.clone()).await.expect("Failed to download Paper.");
    } else if server_type.parse::<i32>().unwrap() == 3 {
        downloaders::fabric::Fabric::download(client.clone()).await.expect("Failed to download Fabric");
    }
